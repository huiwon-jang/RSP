...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def __init__(
        self, global_pool=False, use_cls=True, flatten=False, mask_ratio=None, del_head=True, **kwargs
    ):
        ...
        if global_pool:
            ...
            self.classifier_feature = "flatten"
        else:
    ...
    def handle_outcome(self, x):
        ...
        elif self.classifier_feature == "flatten":
            x = self.norm(x)
            outcome = x.reshape(x.shape[0], -1)
        else:
...
class StochasticViTv1(VisionTransformer):
    def __init__(
        self, global_pool=False, use_cls=True, flatten=False, mask_ratio=None, del_head=True, **kwargs
    ):
        super().__init__(global_pool, use_cls, flatten, mask_ratio, del_head, **kwargs)
        self.to_prior = nn.Sequential(
            nn.Linear(self.embed_dim, self.embed_dim * 2),
            nn.ReLU(),
            nn.Linear(self.embed_dim * 2, 32*32),
        )
    ...
    def forward_features(self, x):
        ...
        cls = self.norm(x)[:, 0]
        prior_logits = self.to_prior(cls)
        max_indices = torch.argmax(prior_logits, dim=-1)
        one_hot = torch.zeros_like(prior_logits)

        # Set the maximum elements to 1
        one_hot.scatter_(-1, max_indices.unsqueeze(-1), 1)
        prior_representation = one_hot.reshape(prior_logits.shape[0], -1)
        return torch.cat([cls, prior_representation], dim=1)


class StochasticViTv2(VisionTransformer):
    def __init__(
        self, global_pool=False, use_cls=True, flatten=False, mask_ratio=None, del_head=True, **kwargs
    ):
        super().__init__(global_pool, use_cls, flatten, mask_ratio, del_head, **kwargs)
    ...
    def forward_features(self, x):
        ...
        return torch.cat([cls, prior_logits], dim=1)


class ViTFlatten(VisionTransformer):
    def __init__(
        self, global_pool=False, use_cls=False, flatten=True, mask_ratio=None, del_head=True, **kwargs
    ):
        super().__init__(global_pool, use_cls, flatten, mask_ratio, del_head, **kwargs)
...
def stochasticv1_vit_small_patch16(**kwargs):
    model = StochasticViTv1(
        patch_size=16,
        embed_dim=384,
        depth=12,
        num_heads=6,
...
def stochasticv2_vit_small_patch16(**kwargs):
    model = StochasticViTv2(
        patch_size=16,
        embed_dim=384,
        depth=12,
        num_heads=6,
...
def flatten_vit_small_patch16(**kwargs):
    model = ViTFlatten(
        patch_size=16,
        embed_dim=384,
        depth=12,
...
def load_moco_encoder(model, checkpoint_path=None):
...
def load_dino_encoder(model, checkpoint_path=None):
    if checkpoint_path is None:
        return model
    else:
        model_utils.download_model_if_needed(checkpoint_path)
    ...
    state_dict = torch.load(checkpoint_path, map_location="cpu")["student"]
    state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
    state_dict = {k.replace("backbone.", ""): v for k, v in state_dict.items()}
    state_dict = {k.replace("base_encoder.", ""): v for k, v in state_dict.items()}
...
def load_simclr_encoder(model, checkpoint_path=None):
    if checkpoint_path is None:
        return model
    else:
        model_utils.download_model_if_needed(checkpoint_path)
