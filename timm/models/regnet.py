...
def downsample_avg(in_chs, out_chs, kernel_size, stride=1, dilation=1, norm_layer=None):
    ...
    return nn.Sequential(
        *[
            pool,
            ConvBnAct(
                in_chs, out_chs, 1, stride=1, norm_layer=norm_layer, act_layer=None
            ),
        ]
    )
    ...
    def __init__(
        ...
        for i in range(depth):
            ...
            if (block_in_chs != out_chs) or (block_stride != 1):
                proj_block = downsample_conv(
                    block_in_chs, out_chs, 1, block_stride, block_dilation
                )
            else:
                proj_block = None
...
class RegNet(nn.Module):
    ...
    def __init__(
        ...
        for i, stage_args in enumerate(stage_params):
            ...
            self.add_module(
                stage_name, RegStage(prev_width, **stage_args, se_ratio=se_ratio)
            )
    ...
    def _get_stage_params(
        self, cfg, default_stride=2, output_stride=32, drop_path_rate=0.0
    ):
        ...
        stage_dpr = np.split(
            np.linspace(0, drop_path_rate, d), np.cumsum(stage_depths[:-1])
        )

        # Adjust the compatibility of ws and gws
        stage_widths, stage_groups = adjust_widths_groups_comp(
            stage_widths, stage_bottle_ratios, stage_groups
        )
        param_names = [
            "out_chs",
            "stride",
