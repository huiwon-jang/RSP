...
default_cfgs = {
    ...
    "cspresnext50": _cfg(
        ...
        pool_size=(7, 7),
        crop_pct=0.875,  # FIXME I trained this at 224x224, not 256 like ref impl
    ),
...
class ResBottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = ConvBnAct(
            mid_chs, mid_chs, kernel_size=3, dilation=dilation, groups=groups, **ckwargs
        )
        self.attn2 = (
...
class DarkBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = ConvBnAct(in_chs, mid_chs, kernel_size=1, **ckwargs)
        self.conv2 = ConvBnAct(
            mid_chs, out_chs, kernel_size=3, dilation=dilation, groups=groups, **ckwargs
        )
...
def _cfg_to_stage_args(cfg, curr_stride=2, output_stride=32, drop_path_rate=0.0):
    ...
    cfg["block_dpr"] = (
        ...
        if not drop_path_rate
            ...
            x.tolist()
            for x in torch.linspace(0, drop_path_rate, sum(cfg["depth"])).split(
                cfg["depth"]
            )
        ]
    )
...
def cspresnext50_iabn(pretrained=False, **kwargs):
    ...
    return _create_cspnet(
        "cspresnext50_iabn", pretrained=pretrained, norm_layer=norm_layer, **kwargs
    )
...
def cspdarknet53(pretrained=False, **kwargs):
    return _create_cspnet(
        "cspdarknet53", pretrained=pretrained, block_fn=DarkBlock, **kwargs
    )
